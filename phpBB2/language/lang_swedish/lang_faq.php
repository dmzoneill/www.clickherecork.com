...
$faq[] = array("Varför kan jag inte logga in?", "Har du registrerat dig? Du måste registrera dig för att kunna logga in. Har du blivit avstängd från forumet (i så fall visas ett meddelande)? Om så är fallet bör du kontakta webbmastern eller forum administratören för att få reda på varför. Om du har registrerat dig och inte är avstängd men ändå inte kan logga in kontrollera då ditt användarnamn och lösenord. Oftast är det problemet, om inte så kontakta forum administratören.");
$faq[] = array("Varför måste jag registera mig?", "Det är inte säkert att du behöver, det är upp till forum administratören om du behöver registrera dig för att skriva ett meddelande. Dock så ger registreringen dig ytterligare funktioner som inte är tillgängliga för gäster, till exempel avatar bilder, personliga meddelanden, e-post till andra användare, prenumeration av användargrupper, etc. Det tar endast några sekunder att registrera sig så det är att rekommendera.");
...
$faq[] = array("Hur hindrar jag att mitt användarnamn visas på Vem är Online-listan?", "I din profil finns en inställning <i>Dölj din online status</i>, om du ändrar inställningen till <i>på</i>, så syns du bara för forum-administratörer och dig själv. Du kommer att räknas som en dold användare.");
...
$faq[] = array("Hur ändrar jag mina inställningar?", "Alla dina inställningar (om du har registrerat dig) är lagrade i databasen. För att ändra dom klicka på Profillänken (visas oftast högst upp på sidan). Detta låter dig ändra alla dina inställningar.");
...
$faq[] = array("Hur ändrar jag min rank?", "I normala fall kan du inte ändra din rang direkt (rang står under ditt användarnamn i ämnen och i din profil beroende på vilken stil som valts). De flesta forumen använder rang för att indikera antalet inläg du har skrivet och för att identifiera vissa användare, e.g. moderatorer och administratörer kan ha en speciell rang. Försök att inte missbruka forumet genom att skriva onödiga inlägg bara för att öka din rang, administratörerna kommer i såfall att helt enkelt sänka ditt antal inlägg.");
$faq[] = array("När jag klickar på e-postlänken till en användare så vill forumet att jag loggar in?", "Tyvärr kan bara registrerade användaren skicka e-post till andra användare vid det inbyggda e-post formuläret (om administratörerna har aktivera denna finess). Detta är för att förhindra missbruk av e-postsystemet av anonyma användare.");
...
$faq[] = array("Kan jag använda HTML?", "Det beror på om administratören tillåter dig, de har total kontroll över det. Om det är tillåtet kommer du att finna att det bara är vissa taggar som fungerar. Detta är en <i>säkerhets</i> åtgärd för att hindra folk från att missbruka forumet genom att använda taggar som kan förstöra designen eller skapa andra problem. Om HTLM är tillåt så kan du hindra det i det aktuella meddelandet när du skapar det.");
...
$faq[] = array("Kan jag posta bilder?", "Bilder kan visas i ditt meddelande. Dock finns det för tillfället ingen funktion för att ladda upp bilder till forumet. Därför måste du länka till en bild som lagras på en publik webserver, t.ex. <pre>http://www.some-unknown-place.net/my-picture.gif</pre>. Du kan inte länka till bilder som lagras på din lokala PC (såvida den inte är en publikt tillgänglig server) eller till bilder som lagras bakom auktoriserings mekanismer, e.g. hotmail eller yahoo mailkonto, lösenordsskyddade siter, etc. För att visa en bild så använd antingen BBCode [img] taggen eller motsvarande HTML (om det tillåts).");
$faq[] = array("Vad är Viktiga ämnen/meddelanden?", "Viktiga meddelande innehåller oftast viktig information och du bör läsa dom så fort som möjligt. Viktiga meddelande syns högst upp på varje sida i det forum som de skrivs. Om du kan eller inte kan skriva viktiga meddelande beror på vilka rättigheter som behövs, vilka styrs av administratörerna.");
$faq[] = array("Vad är Klistrade ämnen/meddelanden?", "Klistrade ämnen syns under viktiga meddelande och enbart på första sidan. De innehåller ofta viktig information så du bör läsa dom när det är möjligt. Såsom med viktiga meddelande så är det administratörerna som bestämmer vilka rättigheter som behövs för att kunna skriva Klistrade ämnen.");
$faq[] = array("Vad är låsta ämnen/meddelanden?", "Låsta ämnen skapas av antingen forum moderatorerna eller av forum-administratörerna. Du kan inte besvara ett låst ämne och om den innehåller en omröstning så stoppas denna automatiskt. Ämnen kan låsas av många skäl.");


$faq[] = array("--","Användarnivåer och grupper");
$faq[] = array("Vad är administratörer?", "Administratörer är personer som har den absolut högst nivån av kontroll över hela forumet. Dessa personer kan kontrollera alla aspekter av forumets drift vilket inkluderar att sätta rättigheter, bannlysa användare, skapa användargrupper eller moderatorer, etc. De har också fulla moderator rättigheter i alla forum.");
...
$faq[] = array("Vad är användargrupper?", "Användargrupper är ett sätt som forum-administratörerna använder för att gruppera användare. Varje användare kan tillhöra flera grupper (detta skiljer från de flesta andra forumen) och varje grupp kan tilldelas individuella rättigheter. Detta gör det enkelt för administratörer att ange flera användare som moderatorer för ett forum eller ge dom åtkomst till ett privat forum, etc.");
$faq[] = array("Hur går jag med i en användargrupp?", "För att ansluta dig till en användargrupp så klicka på användargrupplänken i sidhuvudet (beroende på designmallen som används), du kan då visa alla användargrupper. Inte alla grupper är <i>öppna</i>, vissa är stängda och vissa kan även vara dolda. Om gruppen är öppen kan du ansöka om att få bli medlem genom att klicka på lämplig knapp. Användargruppens moderator måste då i sin tur bevilja din ansökan och kan även fråga varför du vill bli medlem. Och tjata inte på moderatorerna om de nekar dig de har säkerligen sina skäl.");
$faq[] = array("Hur blir jag moderator för en användargrupp?", "Användargrupper skapas initialt av administratörerna och de utser också gruppmoderator. Om du är intresserad av att skapa en användargrupp så är din första kontakt någon av administratörerna. Skicka ett personligt meddelande till de.");


$faq[] = array("--","Personliga meddelanden");
$faq[] = array("Jag kan inte skicka personliga meddelanden!", "Det finns tre skäl till detta; du är inte registrerad och/eller du har inte logga in, administratören har inaktiverat personliga meddelanden för hela forumet eller administratörern har stoppat dig från att skicka meddelanden. Om det är det senare fallet så fråga administratören varför.");
$faq[] = array("Jag får oönskade personliga meddelanden!", "I framtiden kommer vi att lägga till en ignoreralista till det personliga meddelandesystemet. Om du fortsätter att få oönskade meddelanden så prata med en administratör, de har möjligheter att stoppa en användare från att skicka personliga meddelanden överhuvudtaget.");
$faq[] = array("Jag har fått spam eller anstötliga e-postmeddelanden från någon på detta forum!", "Vi är ledsna att höra detta. E-postforumläret i forumet innehåller skydd för att försöka att spåra användare som skickar sådana e-postmeddelanden. Du bör e-posta administratörerna på forumet och bifoga en full kopia av e-postmeddelande du fick och det är mycket viktigt att du bifogar e-posthuvudet (detta innehåller detaljerna om vilken användare som skickat e-postmeddelandet). Administratörerna kan därefter vidta åtgärder.");
