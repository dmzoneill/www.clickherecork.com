...
print "<h4>Sales Analysis</h4><a href='index.php'>Main Menu</a> >> <a href='sales.php'>Sales</a>";
...
print "<h3>Over view for $day of the $month $year...</h3>";

	$sql = $stream->do_query("SELECT * FROM `clicksales` WHERE rday='$day' AND rmonth='$month' AND ryear='$year' order by id asc","array");
...
$tmon = date("m");
	$sqlf = $stream->do_query("SELECT * FROM `clicksales` where rmonth=$tmon order by rday ASC","array");
	$targa = 0;
	$p=0;
	...
	for($t=0;$t<count($sqlf);$t++){
		...
		if($targa!=$dday){
			print "<a href='sales.php?day=$dday&month=$dmon&year=$dyear'> $dday - $dmon - $dyear </a><br>";
			$targa = $dday;
			$p++;
		}
		else {
	...
	$sqlf = $stream->do_query("SELECT * FROM `clicksales` order by rday DESC","array");
	$targa = 0;
	$p=0;
