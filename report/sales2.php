...
print "<h4>Sales Analysis</h4><a href='index.php'>Main Menu</a>";
...
if(($day) && ($month) && ($year)){
	...
	$sql = $stream->do_query("SELECT * FROM `clicksalestest` WHERE rday='$day' AND rmonth='$month' AND ryear='$year' order by id asc","array");
	...
	print "* Note : <br>The figures provided by this web page are only an estimate.<br><br> 
	...
	3. Loyalty Cards effectivley void some transactions.<br> 4. ETC<br><br><hr><br>";

	print "<h3>Select a date</h3><form action='sales.php' method=post>";
	print " Day : <input size='4' name='day' value='$yday' type='text'> ";
	print " Month : <input size='4'  name='month' value='$ymon' type='text'> ";
	print " Year : <input size='4'  name='year' value='$yyear' type='text'> ";
	print " <input type=submit value='Select a date'> ";
	print "</form><br><br>";

	print "<table width=500 cellpadding=5 border=0><tr>";
	
	print "<td valign=top><br>Last 10 available dates<br><br>";

	$sqlf = $stream->do_query("SELECT * FROM `clicksalestest` order by rday DESC","array");
	$targa = 0;
	$p=0;
	
	for($t=0;$t<count($sqlf);$t++){
		$tmp = $sqlf[$t];
		$dday = $tmp[1];
		$dmon = $tmp[2];
		$dyear = $tmp[3];
		...
		if($targa!=$dday){
			print "<a href='sales2.php?day=$dday&month=$dmon&year=$dyear'> $dday - $dmon - $dyear </a><br>";
			$targa = $dday;
			$p++;
		}
		else {
	...
	print "</td><td valign=top><br>Monthly Over View<br><br>";
	...
	for($t=0;$t<count($sqlf);$t++){
		...
		if($targa!=$dmon){
			print "<a href='monthly.php?month=$dmon&year=$dyear'> $dmon - $dyear </a><br>";
			$targa = $dmon;
			$p++;
		}
		else {
